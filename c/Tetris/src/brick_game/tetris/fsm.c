...
void sigact(UserAction_t sig, state_s *state, wins *win, GameInfo_t *g_info,
...
void start(params_t *p) {
  init_stats(p->g_info);
  updateCurrentState(p->g_info, p->win);
  *p->state = SPAWN;
}
...
void shifting(params_t *p) {
  ...
  if (p->sh->shift_down + p->list[p->sh->num_shape].down + 1 == 19) {
    ...
    if (check_next_rows(*p->g_info, p->list, *p->sh, 0))
      ...
      *p->state = MOVING;
    remove_block(p->g_info, p->list, p->sh);
    if (p->sh->delay++ == 5) p->sh->shift_down++, p->sh->delay = 0;
    add_figure_in_place(p->g_info, p->list, p->sh);
  }
  updateCurrentState(p->g_info, p->win);
}
void attaching(params_t *p) {
  check_line(p->g_info, p->win);
  *p->state = SPAWN;
  if (check_last_line(p->g_info)) *p->state = GAME_OVER;
}
void exit_app(params_t *p) { exit_game(*p->g_info, p->win, p->sh); }
