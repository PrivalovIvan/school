...
void exit_game(GameInfo_t g_info, wins *win, shapes_s *sh) {
  for (int j = 0; j < 20; j++) {
    for (int k = 0; k < 10; k++) {
      g_info.field[j][k] = 1;
  ...
  int rows1 = 0, rows2 = 19;
  while (rows1 < 10) {
    wclear(win->win_game);
    mvwprintw(win->win_game, rows1++, 4, "GAME");
    mvwprintw(win->win_game, rows2--, 4, "OVER");
...
int check_line(GameInfo_t *g_info, wins *win) {
  ...
  for (i = 0; i < 20; i++) {
    ...
    if (line) {
      line_qty++;
      for (int k = 0; k < 10; k++)
        g_info->field[i][k] = 0, line = 0, updateCurrentState(g_info, win),
        msleep(75);
      for (int a = i; a > 1; a--) {
        for (int b = 0; b < 10; b++) {
          if (g_info->field[a - 1][b] == 1) g_info->field[a][b] = 1;
          if (g_info->field[a - 1][b] == 0) g_info->field[a][b] = 0;
        }
      }
      if (line_qty == 1)
...
void level_up(GameInfo_t *g_info) {
  ...
  if (g_info->level > 10) g_info->level = 10;
}
...
void write_hi_score(GameInfo_t g_info) {
  ...
  if (fp) {
    ...
    for (int i = 0; i < n; i++) putc(str[i], fp);
    fclose(fp);
  }
}
