...
START_TEST(pow_test_3) {
  ...
  ck_assert_double_eq(pow(0, -1), s21_pow(0, -1));                  // inf +
  ck_assert_double_eq(pow(0, 1), s21_pow(0, 1));                    //  0 +
  ck_assert_double_eq(pow(INFINITY, 0), s21_pow(INFINITY, 0));      // 1  +
  ck_assert_double_eq(pow(INFINITY, -1), s21_pow(INFINITY, -1));    // 0  +
  ck_assert_double_eq(pow(-1, -INFINITY), s21_pow(-1, -INFINITY));  // 1  +
  ck_assert_double_eq(pow(0, INFINITY), s21_pow(0, INFINITY));      // 0 +
  ck_assert_double_nan(s21_pow(0, NAN));                            // nan --
  ck_assert_double_eq(pow(NAN, 0), s21_pow(NAN, 0));                // 1
  ck_assert_double_nan(s21_pow(NAN, NAN));                          // nan
  ck_assert_double_eq(pow(INFINITY, INFINITY),
                      s21_pow(INFINITY, INFINITY));  // inf --
  ck_assert_double_eq(pow(INFINITY, -INFINITY),
                      s21_pow(INFINITY, -INFINITY));  // 0 --
  ck_assert_double_eq(pow(-INFINITY, INFINITY),
                      s21_pow(-INFINITY, INFINITY));  // inf --
  ck_assert_double_eq(pow(-INFINITY, -INFINITY),
                      s21_pow(-INFINITY, -INFINITY));                 // 0 --
  ck_assert_double_eq(pow(1, -INFINITY), s21_pow(1, -INFINITY));      // 1 --
  ck_assert_double_eq(pow(1, NAN), s21_pow(1, NAN));                  // 1 --
  ck_assert_double_nan(s21_pow(NAN, INFINITY));                       // nan
  ck_assert_double_nan(s21_pow(INFINITY, NAN));                       // nan
  ck_assert_double_nan(s21_pow(NAN, -INFINITY));                      // nan
  ck_assert_double_nan(s21_pow(-INFINITY, NAN));                      // nan
  ck_assert_double_eq(pow(2, INFINITY), s21_pow(2, INFINITY));        // inf
  ck_assert_double_eq(pow(0.5, INFINITY), s21_pow(0.5, INFINITY));    //
  ck_assert_double_eq(pow(-2, INFINITY), s21_pow(-2, INFINITY));      // inf --
  ck_assert_double_eq(pow(2, -INFINITY), s21_pow(2, -INFINITY));      //
  ck_assert_double_eq(pow(0.5, -INFINITY), s21_pow(0.5, -INFINITY));  // inf
  ck_assert_double_eq(pow(-2, -INFINITY), s21_pow(-2, -INFINITY));    // 0
}
END_TEST
...
START_TEST(sin_test_1) {
  for (int R = 20; R >= -20; R -= 4) {
    for (double k = 2 * M_PI; k > -2 * M_PI; k -= M_PI / 17) {
      ck_assert_double_eq_tol(sin(k + R * M_PI), s21_sin(k + R * M_PI), 1e-7);
    }
  }
}
END_TEST
...
START_TEST(cos_test_1) {
  for (int R = 20; R >= -20; R -= 4) {
    for (double k = 2 * M_PI; k > -2 * M_PI; k -= M_PI / 17) {
      ck_assert_double_eq_tol(cos(k + R * M_PI), s21_cos(k + R * M_PI), 1e-7);
    }
  }
}
END_TEST
...
START_TEST(tan_test_1) {
  for (int R = 2.5; R >= -2.5; R -= 2.5) {
    for (double k = M_PI; k > -M_PI; k -= M_PI / 9) {
      ck_assert_double_eq_tol(tan(k + R * M_PI), s21_tan(k + R * M_PI), 1e-7);
    }
  }
}
END_TEST
...
START_TEST(exp_test_4) {
  ...
  ck_assert_ldouble_eq_tol(exp(1), s21_exp(1), 1e-10);
}
END_TEST
