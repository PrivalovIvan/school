...
//     if (state == GAME_OVER)
//       exit_game(g_info), sh.run = FALSE;

//     sigact(get_signal(signal), &state, &g_info, list, &sh);

//     if (state == START or state == MOVING)
//       signal = wgetch(win.win_game);

//     if (state == MOVING and signal == -1)
//       state = SHIFTING;
//   }
...
// //                 if (state == GAME_OVER) exit_game(g_info, &win, &sh),
// sh.run
// //                 = FALSE;

// //         sigact(get_signal(signal), &state, &win, &g_info, list, &sh);

// //         if (state == START or state == MOVING) signal =
// wgetch(win.win_game);
